...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card" style="border-radius: 10px;">
            ...
            <div class="body">
                <form action="{{ route('billing.setting.update') }}" method="POST">
                    @csrf
                    @method('PUT')

                    <label for="expired_at">Jatuh Tempo (expired_at)</label>
                    <div class="form-group">
                        <div class="form-line">
                            <input type="datetime-local" name="expired_at" class="form-control"
                                   value="{{ old('expired_at', optional($billing->expired_at ?? null)->format('Y-m-d\TH:i')) }}" required />
                        </div>
                        <small class="text-muted">Zona waktu Asia/Jakarta.</small>
                    </div>
                    ...
                    <div class="m-t-20">
                        ...
                        <a href="{{ url('/') }}" class="btn btn-default waves-effect">Kembali</a>
                    </div>
                </form>
            </div>
        </div>
